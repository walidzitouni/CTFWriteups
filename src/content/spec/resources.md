# Security Resources

A curated collection of tools, platforms, and resources for CTF competitions and security research.

---

## Learning Platforms

| Resource | Description |
|----------|-------------|
| [HackTheBox](https://www.hackthebox.com/) | Online platform for practicing penetration testing skills |
| [TryHackMe](https://tryhackme.com/) | Guided cybersecurity training with hands-on labs |
| [PortSwigger Web Security Academy](https://portswigger.net/web-security) | Free web security training from Burp Suite creators |
| [PentesterLab](https://pentesterlab.com/) | Hands-on web penetration testing exercises |
| [OverTheWire](https://overthewire.org/wargames/) | Wargames to learn security concepts |
| [picoCTF](https://picoctf.org/) | Beginner-friendly CTF platform |
| [Root-Me](https://www.root-me.org/) | Challenges and virtual environments |
| [pwnable.kr](http://pwnable.kr/) | Pwn challenges for binary exploitation |

---

## Web Security

| Tool | Description |
|------|-------------|
| [Burp Suite](https://portswigger.net/burp) | Web application security testing |
| [OWASP ZAP](https://www.zaproxy.org/) | Open source web app scanner |
| [SQLMap](https://sqlmap.org/) | Automatic SQL injection tool |
| [Nikto](https://github.com/sullo/nikto) | Web server scanner |
| [Gobuster](https://github.com/OJ/gobuster) | Directory/DNS busting tool |
| [ffuf](https://github.com/ffuf/ffuf) | Fast web fuzzer |
| [CSP Evaluator](https://csp-evaluator.withgoogle.com/) | Content Security Policy analyzer |
| [JWT.io](https://jwt.io/) | JWT decoder and debugger |
| [RequestBin](https://requestbin.com/) | HTTP request collector |
| [Webhook.site](https://webhook.site/) | Webhook testing and debugging |

---

## Cryptography

| Tool | Description |
|------|-------------|
| [CyberChef](https://gchq.github.io/CyberChef/) | The cyber swiss army knife - encoding, encryption, compression |
| [dCode](https://www.dcode.fr/en) | Cipher identifier and decoder |
| [CrackStation](https://crackstation.net/) | Online hash cracking |
| [FactorDB](http://factordb.com/) | Integer factorization database |
| [RsaCtfTool](https://github.com/RsaCtfTool/RsaCtfTool) | RSA attack tool |
| [QuipQiup](https://quipqiup.com/) | Substitution cipher solver |
| [Boxentriq](https://www.boxentriq.com/code-breaking) | Code breaking tools collection |
| [SageMath](https://www.sagemath.org/) | Mathematical software for crypto |

---

## Forensics & Steganography

| Tool | Description |
|------|-------------|
| [Aperi'Solve](https://www.aperisolve.com/) | Online steganography analysis |
| [StegOnline](https://stegonline.georgeom.net/upload) | Online image steganography tool |
| [Stegsolve](https://github.com/zardus/ctf-tools/tree/master/stegsolve) | Image analysis for hidden data |
| [Binwalk](https://github.com/ReFirmLabs/binwalk) | Firmware analysis and extraction |
| [Foremost](https://github.com/korczis/foremost) | File recovery tool |
| [Volatility](https://www.volatilityfoundation.org/) | Memory forensics framework |
| [Autopsy](https://www.autopsy.com/) | Digital forensics platform |
| [Wireshark](https://www.wireshark.org/) | Network protocol analyzer |
| [Exiftool](https://exiftool.org/) | Metadata reader/writer |
| [zsteg](https://github.com/zed-0xff/zsteg) | PNG/BMP steganography detector |

---

## OSINT

| Tool | Description |
|------|-------------|
| [Shodan](https://www.shodan.io/) | Search engine for Internet-connected devices |
| [Censys](https://censys.io/) | Internet-wide scanning and search |
| [Wayback Machine](https://web.archive.org/) | Internet archive for historical snapshots |
| [Hunter.io](https://hunter.io/) | Email finder tool |
| [Sherlock](https://github.com/sherlock-project/sherlock) | Username search across social networks |
| [theHarvester](https://github.com/laramies/theHarvester) | OSINT gathering tool |
| [OSINT Framework](https://osintframework.com/) | Collection of OSINT tools |
| [Maltego](https://www.maltego.com/) | Interactive data mining |

---

## Reverse Engineering

| Tool | Description |
|------|-------------|
| [Ghidra](https://ghidra-sre.org/) | NSA's reverse engineering suite |
| [IDA Free](https://hex-rays.com/ida-free/) | Interactive disassembler |
| [Binary Ninja](https://binary.ninja/) | Reverse engineering platform |
| [Radare2](https://rada.re/n/) | Open source RE framework |
| [Dogbolt](https://dogbolt.org/) | Online decompiler explorer |
| [GDB](https://www.gnu.org/software/gdb/) | GNU debugger |
| [pwndbg](https://github.com/pwndbg/pwndbg) | GDB plugin for exploit dev |
| [x64dbg](https://x64dbg.com/) | Windows debugger |

---

## Password Cracking

| Tool | Description |
|------|-------------|
| [Hashcat](https://hashcat.net/hashcat/) | Advanced password recovery |
| [John the Ripper](https://www.openwall.com/john/) | Password cracker |
| [CrackStation](https://crackstation.net/) | Online hash lookup |
| [Hashes.com](https://hashes.com/en/decrypt/hash) | Hash decryption service |
| [SecLists](https://github.com/danielmiessler/SecLists) | Security wordlists collection |
| [RockYou](https://github.com/brannondorsey/naive-hashcat/releases/download/data/rockyou.txt) | Famous password wordlist |

---

## CTF Platforms & Wargames

| Platform | Description |
|----------|-------------|
| [HackTheBox](https://www.hackthebox.com/) | Penetration testing labs |
| [TryHackMe](https://tryhackme.com/) | Guided learning paths |
| [VulnHub](https://www.vulnhub.com/) | Vulnerable VMs for practice |
| [CTFtime](https://ctftime.org/) | CTF event calendar and rankings |
| [picoCTF](https://picoctf.org/) | Educational CTF |
| [CryptoHack](https://cryptohack.org/) | Cryptography challenges |
| [Pwnable.xyz](https://pwnable.xyz/) | Pwn challenges |
| [Exploit Education](https://exploit.education/) | Binary exploitation learning |

---

## Useful Utilities

| Tool | Description |
|------|-------------|
| [CyberChef](https://gchq.github.io/CyberChef/) | Data transformation tool |
| [Regex101](https://regex101.com/) | Regular expression tester |
| [RevShells](https://www.revshells.com/) | Reverse shell generator |
| [GTFOBins](https://gtfobins.github.io/) | Unix binary exploitation |
| [LOLBAS](https://lolbas-project.github.io/) | Windows living-off-the-land binaries |
| [PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings) | Payload collection |
| [HackTricks](https://book.hacktricks.xyz/) | Hacking tricks encyclopedia |
| [ExploitDB](https://www.exploit-db.com/) | Exploit database |

---

## Cheatsheets

| Resource | Description |
|----------|-------------|
| [Cheatsheet-God](https://github.com/OlivierLaflamme/Cheatsheet-God) | Massive cheatsheet collection |
| [CTF Tools](https://github.com/gregalletti/CTF_tools) | CTF tools compilation |
| [PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings) | Web attack payloads |
| [HackTricks](https://book.hacktricks.xyz/) | Pentesting methodology |
| [OWASP Cheat Sheet](https://cheatsheetseries.owasp.org/) | Security best practices |
|[Search CTF Writeups](https://ctfsearch.hackmap.win/) | CTF DataBase |

---

> "The quieter you become, the more you can hear." - Ram Dass
